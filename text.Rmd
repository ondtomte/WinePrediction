...
linear.mse <- lapply(dataSets, function(x){
  model<-glm(quality~.,data=as.data.frame(x$train))
...
lasso.models <- lapply( dataSets, function(x){
  t<-as.data.frame(x$train)
  v<- as.data.frame(x$validation)
  m<-as.matrix(t[-ncol(t)])
  models<-lars(x = m, y = as.numeric(t$quality), type = "lasso")
  
  cv<-cv.lars(x = m, y = as.numeric(t$quality), type = "lasso", plot.it = FALSE, K = 5)
  pred<-predict(models, as.matrix(v))
  mse<-sapply(1:ncol(pred$fit),function(x){MSE(pred$fit[,x],validation$quality)})
  x=list(validationMSE=mse,CV=cv, model=models)
})
```
...
library(MASS)
ridge.mse<-lapply(dataSets[-14],function(x){
  #x<- dataSets$poly2
  t<-data.frame(x$train)
  v<-data.frame(x$validation)
  models<-lm.ridge(formula = quality~.,data=t,lambda = seq(0,1500,by=10))
  predictions <- scale(v,center = models$xm,scale = models$scales)%*%models$coef +models$ym
  validatioMSE <- apply(predictions,MARGIN=2,MSE,y=validation$quality)
  
  predictions<-  scale(t[-ncol(t)],center = models$xm,scale = models$scales)%*%models$coef +models$ym
  trainMSE<-apply(predictions,MARGIN=2,MSE,y=train$quality)
  list(trainMSE=trainMSE, validationMSE= validatioMSE, model=models)
})
plot(ridge.mse$poly3$trainMSE,type="l",ylim = c(0.35,0.58))
lines(ridge.mse$poly3$validationMSE,type="l")

plot(ridge.mse$poly2$trainMSE,type="l",ylim = c(min(ridge.mse$poly2$trainMSE),max(ridge.mse$poly2$validationMSE)))
lines(ridge.mse$poly2$validationMSE,type="l")

plot(ridge.mse$pca6$trainMSE,type="l",ylim = c(min(ridge.mse$pca6$trainMSE),max(ridge.mse$pca6$validationMSE)))
lines(ridge.mse$pca6$validationMSE,type="l")
...
tps <- lapply(dataSets, function(x){
  ...
  list(validationMSE=MSE(pred,validation[12]),trainMSE=mean(model$residuals^2))
})
```

```{r}
require(randomForest)
rf <- lapply(dataSets, function(x){
  model<-randomForest(quality ~ ., data=as.data.frame(x$train), ntree = 500)
  tuned.model <- randomForest(quality ~ ., data=as.data.frame(x$train), ntree =
                                which.min(model$mse))
  pred<-predict(model,as.data.frame(x$validation))
  list(validationMSE=MSE(pred,validation[12]),trainMSE=tuned.model$mse[length(tuned.model$mse)])
})
```
...
dd<-distMatrix(as.matrix(train[-12]),as.matrix(validation[-12]))
...
local.regression <- function(k, d){
  ...
  if (is.na(remove[1])) {validationMSE <- MSE(pred, validation[,12])
  } else if (is.na(remove[1]) == FALSE) {validationMSE <- MSE(pred[-remove], validation[-remove,12])}

return(list(validationMSE))}


mse<-c()
for(r in seq(50,200,by=50)){
  pred<-c()
  for(i in 1:nrow(validation)){
    n<- sort(dd[i,],index.return = TRUE)$ix[1:r]
    mod<-lm(quality~.,data=train[n,])
    pred<-c(pred,predict(mod,newdata=validation[i,]))
  }
  mse<-c(mse,MSE(pred,validation$quality))
}
