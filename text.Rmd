...
c.plot <- function(x){
  for (j in c("acc","prec","rec")){
    ...
    plot(performance(x$normal, j), col = 1, xlim = c(0,1.2))
    for (i in 2:length(x)){
  ...
  performance(x[[1]], "prec")@y.values[[1]] + performance(x[[1]],"rec")@y.values[[1]], col =
  1, xlim = c(0,1.2), type = "l", xlab = "Cutoff", ylab = "Accuracy + Precision + Recall")
  
  for (i in 2:length(x)){
    lines(performance(x[[i]], "acc")@x.values[[1]], performance(x[[i]], "acc")@y.values[[1]] +
    performance(x[[i]], "ppv")@y.values[[1]] + performance(x[[i]], "tpr")@y.values[[1]], col = i)}
  legend("topright", names(x), col=seq(1,length(x)), lty=1, cex = 0.5)
}
```