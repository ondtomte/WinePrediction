...
```{r}
require(FNN)
fnn.models<- lapply(dataSets,function(x){
  mce<-sapply(c(5,10,20), function(k){
    pred<-knn(train = x$train[-ncol(x$train)],test = x$validation,cl = x$train$binary.quality,k=k)
    MCE(pred,validation$binary.quality)
  })
  list(mce=mce,best=list(bestMce=min(mce)),k=c(5,10,20)[which.min(mce)])
})
mce<-sapply(fnn.models, function(x){
  x$best$bestMce
})
barplot(mce,las=2)
```
...
require(randomForest)
rf <- lapply(dataSets, function(x){
  model<-randomForest(binary.quality ~ ., x$train, ntree = 30)
...
# accuarcy
plot(performance(logistic.reg$normal, "acc"), col = 6, xlim = c(0,1.2))
for (i in 2:14){
  lines(performance(logistic.reg[[i]], "acc")@x.values[[1]], performance(logistic.reg[[i]], "acc")@y.values[[1]], col = i + 5)}

legend("topright", c("normal","poly2","poly3","PCA","PCA10","PCA9","PCA8","PCA7","PCA6","PCA5","PCA4","PCA3","PCA2", "PCA1"), col=seq(6,14), lty=1, cex = 0.5)


# precision
plot(performance(logistic.reg$normal, "ppv"), col = 6, xlim = c(0,1.2))
for (i in 2:14){
  lines(performance(logistic.reg[[i]], "ppv")@x.values[[1]], performance(logistic.reg[[i]], "acc")@y.values[[1]], col = i + 5)}

legend("topright", c("normal","poly2","poly3","PCA","PCA10","PCA9","PCA8","PCA7","PCA6","PCA5","PCA4","PCA3","PCA2", "PCA1"), col=seq(6,14), lty=1, cex = 0.5)

# precision
plot(performance(logistic.reg$normal, "tpr"), col = 6, xlim = c(0,1.2))
for (i in 2:14){
  lines(performance(logistic.reg[[i]], "tpr")@x.values[[1]], performance(logistic.reg[[i]], "acc")@y.values[[1]], col = i + 5)}
