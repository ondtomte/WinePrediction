...
In the regression subtask we are of course seeking to minimize the MSE (mean squared error) as far as possible. Moreover, we definitely want our regression model to give a lower MSE than the MSE we obtain in our baseline model, this model will consist of simply guessing the mean quality of the training.
...
The models performed badly, the only model with descent performance was the alcohol model (note that alcohol occured frequently in the correlation scores) with a validation MSE of `r mse.lm.models[11]` corresponding to a `r proc.base(mse.lm.models[11])`% procentual improvement from baseline.
...
Poly2 performs best with a validation MSE of `r mse.linearV[2]` (`proc.base(mse.linearV[2])`% improvement over baseline).
...
The best ridge model was poly3 with $\lambda$ equal to `r seq(0,1500,by=10)[which.min(ridge.models$poly3$validationMSE)]`, this model gave a validation MSE of `r mse.ridgeV[3]` (`proc.base(mse.ridgeV[3])`% better than baseline).
...
We constructed our local regression models with 30 to 590 neighbors, stepping by 80. The best model was the normal data set with 190 neighbors, giving a validation MSE of `loc.reg.mse$normal[3]` corresponding to a `r proc.base(loc.reg.mse$normal[3])`% improvement of baseline.
...
plot(seq(10, 1000, 50), valid, type = "l", col = "red", ylab = "Validation MSE", xlab = "Number of trees", ylim = c(0.05,0.42))
...
We now compare the different regression models against each other:
...
Our chosen model is therefore the random forest model with the normal feature set and having 210 trees.
...
The obtained test MSE is `proc.base(final.rf.model.list$testMSE)`% lower than our baseline, and thus satisifies our goal of a 50% MSE reduction. The training MSE of our chosen model is very close to zero, suggesting a risk for overfitting, however the validation and test MSEs are low and very close to each other, implying that the model seem to generalize well to unseen data sets, the following left plot show these MSEs. To the right we plot how the predicted mean values of the model compares to the actual, we calculate this as an easy way to investigate if our model has a tendency to predict too low or too high values:
...
As expected (since the MSE of our model is very low), the actual means and the predicted means are very close to each other and we can not see any meaningful positive or negative tendency.

#5.3 Learning rate
After finding a good model we wanted to answers if more data would create an even better model. To do this we calculate and plot a diagram with MSE as an function of training data. As you can see the validation MSE is still shrinking fast:
...
plot(y=t,x = toTest*nrow(trainValidate),type="l",col="red",ylim=c(0,0.6),ylab="MSE",xlab="n",main="Learning Rate Random Forest")
...
barplot(rf.mce,las=2,main="Random Forest",ylim = c(0,0.45))

#TPS
tps.mce<- sapply(tps.models,function(x){
  x$mceValidation
})
barplot(tps.mce,las=2,main="TPS",ylim = c(0,0.45))
...
The best KNN model turns out to be the data set poly3 with K = 5, giving a validation MCE of 0.1935047.
...
The following support vector machine (SVM) models find the hyper-plane which best separates the classes given the specified hyper-parameters. Note that we only construct this model with the normal data set due to the large number of hyper-parameters.
...
df[order(as.numeric(df$validationError))[5],]
...
df[order(as.numeric(df$validationError))[3],]
...
```{r,eval=TRUE,echo=FALSE}
generic.plot(iterations, c(20,43), c(.16,.33), "Number of iterations", ada.models)
...
legend("topright", names(x), col=cust.col, lty=1, cex = 0.5)
```
##6.2133423 Learning Rate Best Model for Classification
...
l<-lapply(toTest,function(x){
    ...
    train<-trainValidate[s==TRUE,]
    validate<-trainValidate[s==FALSE,]
    train$binary.quality<-as.factor(train$quality>GOOD_WINE)
    validate$binary.quality<-as.factor(validate$quality>GOOD_WINE)
    train$quality<-NULL
    validate$quality<-NULL
    model<-randomForest(binary.quality ~ ., data=train, ntree = 5)
    tuned.model <- randomForest(binary.quality ~ ., data=train, ntree =
                                which.min(model$err.rate[,1]))
    predValidation<-predict(tuned.model,validate)
    predTrain<-predict(tuned.model,train)
    
    list(validationMCE=MCE(predValidation,validate$binary.quality),
         trainMCE=MCE(predTrain,train$binary.quality))
    })
```
...
plot(y=t,x = toTest*nrow(trainValidate),type="l",col="red",ylim=c(0,0.3),ylab="MCE",xlab="n",main="Learning Rate Random Forest")
lines(y=v,x=toTest*nrow(trainValidate),col="blue")
legend("top",legend = c("Train","Validation"),col=c("red","blue"),lwd = 1)
```
...
```{r,eval=TRUE,cache=TRUE,echo=FALSE}
baseline.MCE.new <- c(baseline.misclass, rep(NA, 13))

ada.models.new <- c(ada.models, NA)

svm.new <- c(df$validationError[[13]], rep(NA, 12))
...
barplot(bar.matrix, beside = TRUE, ylim = c(0,0.8), names = c("Baseline","KNN","LDA","QDA","SVM","RF","ADA", "Log. reg.", "", ""), las = 2, col = cust.col)
legend("topright", names(dataSets), col= cust.col, lty=1, lwd = 5, cex = 0.60)
```

Result on test set
```{r,echo=FALSE}
test$binary.quality<- test$quality>GOOD_WINE
test$quality<- NULL
MCE(predict(randomForest.c.models$normal$model,test),test$binary.quality)
```
