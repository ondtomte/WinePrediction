...
#RF.new <- sapply(randomForest.c.models, function(x){})
RF.new <- rep(NA,13)
#ada.models.new <- c(adamodels, NA)
ada.models.new <- rep(NA,13)


log.new <- c()
for (i in 1:13){
  log.new[i] <- min(1 - performance(x[[i]], "acc")@y.values[[1]])
}

bar.matrix <- matrix(c(baseline.MCE.new, best.knn.models, new.lda.models, new.qda.models, svm.new, RF.new, ada.models.new, log.new, rep(NA, 13), rep(NA, 13)), ncol = 10)
