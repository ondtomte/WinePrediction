...
Our task is to investigate how different chemical levels affect the quality of the wine, and we split this task into two sub tasks:
...
Intuitively PCA defines a set of new base vectors of our data (corresponding to a rotation), the idea is to keep the directions of high variance since these are assumed to be the most informative (at least given a linear relation between the attributes and the target variable).
...
The models performed badly, the only model with descent performance was the alcohol model (note that alcohol occurred frequently in the correlation scores) with a validation MSE of `r round(mse.lm.models[11], 3)` corresponding to a `r round(proc.base(mse.lm.models[11]), 1)`% improvement from baseline.
...
The difference in validation MSE between the best models is quite small. Polynomial regression of degree 3 seems to overfit (since the difference between its training and validation MSE is high compared to the other models), yet still performs second best. It can also be noted that the CV errors (raw and adjusted) lie close to the actual validation errors, as expected. Moreover, the more PCA components we use the better the model gets, suggesting that all variables are important.

The CV performed was 5-fold. We will not perform any more CV because we had problems implementing it and it is also quite computationally expensive.
...
```{r,cache=TRUE,eval=TRUE,warning=FALSE,message=FALSE, echo = FALSE}
require(fields)
...
PCA7 gave the lowest thin plate spline validation MSE, `r round(tps.valid[7], 3)` (a `r round(proc.base(tps.valid[7]), 1)`% improvement over baseline). The PCA5, PCA7 and PCA9 models all have a very low training MSE, in the case of PCA9 this seems to cause overfitting while PCA7 performs surprisingly well, we do not have any explanation of this behavior.

We note that the TPS package returns a warning message regarding the $\lambda$. We believe this warning message is produced because the $\lambda$ selected through cross-validation is very close to 0, if $\lambda$ would be 0 then any function interpolating the data would be acceptable and such a function is not unique.
...
We constructed random forest models with 500 trees, the barplot below shows the result of the tuned models.
...
We continue by plotting how the validation and training MSE for the normal data set is affected by the number of trees (we have not tuned the models from which we have obtained the following results). The blue line represents the training MSE and the red represents validation (note that we do not tune the models in this case):
...
lm.one <- c(lm.models$alcohol$validationMSE, rep(NA,12))

bar.matrix <- matrix(c(baseline.MSE.new, lm.one, mse.linearV, mse.lassoV, mse.ridgeV.new, tps.valid.new, loc.reg.Vmse.new, mse.forestV, rep(NA, 13), rep(NA, 13)), ncol = 10)


barplot(bar.matrix, beside = TRUE, ylim = c(0,0.8), names = c("Baseline", "1. var. LM", "LM","Lasso","Ridge","TPS","Local reg.","RF", "", ""), las = 2, col = cust.col, ylab = "Validation MSE")
...
barplot(best.V, las = 2, col = c(cust.col[1],cust.col[1],cust.col[2],cust.col[3],cust.col[3],cust.col[7],cust.col[1],cust.col[1]), ylim = c(0,0.8), main = "Best Data sets", ylab = "Validation MSE")
...
The obtained test MSE of `r round(final.rf.model.list$testMSE, 3)`  is `r round(proc.base(final.rf.model.list$testMSE), 1)`% lower than our baseline, satisfying our goal of a 50% MSE reduction. The training MSE (`r round(final.rf.model.list$trainMSE, 3)`) of our chosen model is very close to zero, suggesting a risk for overfitting, however the validation (`r round(proc.base(final.rf.model.list$validationMSE),3)`) and test MSEs are low and very close to each other, implying that the model seem to generalize well to unseen data, the following left plot show these MSEs:

```{r,eval=TRUE,cache=TRUE, echo = FALSE}
par(mfrow=c(1,2))

barplot(c("Training" = final.rf.model.list$trainMSE, "Validation" = final.rf.model.list$validationMSE, "Test" = final.rf.model.list$testMSE), las = 2, ylab = "MSE", ylim = c(0,0.40))
...
barplot(mean.matrix, las = 2, ylab = "Mean Quality", beside = TRUE, ylim = c(0,8), names = c("Training","Validation","Test"), col = c(2,3))
...
After finding a good model we wanted to answer if more data would create an even better model. To do this we calculate and plot a diagram with MSE as a function of the number of observations in the training data. As you can see the validation MSE is still shrinking fast (note that we do not tune the models):
...
#                                which.min(model$mse))
    predValidation<-predict(model,validate)
...
This baseline consists of guessing that a new observation belong to the same class as the majority of the observations in the training data, this model gave a MSE of `r round(baseline.misclass, 3)`.
...
```{r,eval=TRUE,cache=TRUE,echo=FALSE, message = FALSE}
require(MASS)
...
```{r,eval=TRUE,cache=TRUE,echo=FALSE, message = FALSE}
lda.qda.matrix <- sapply(lda.qda.models, function(x){c(x$lda.validation.MCE, x$qda.validation.MCE)})

barplot(lda.qda.matrix, beside = TRUE, ylim = c(0,0.35), names = names(dataSets), las = 2, col = c(3,4), ylab = "Validation MCE")
legend("topleft", c("LDA", "QDA"), col= c(3,4), lty=1, lwd = 5, cex = 0.60)
```

The models for poly3 performed best, its LDA model had a validation MCE of `r round(lda.qda.models$poly3$lda.validation.MCE, 3)` (`r round(proc.base.class(lda.qda.models$poly3$lda.validation.MCE),1)`% under baseline) and its QDA model had a MCE of `r round(lda.qda.models$poly3$lda.validation.MCE,3)` (`r round(proc.base.class(lda.qda.models$poly3$qda.validation.MCE),1)`% under baseline).
...
We construct a set of random forest models (note that we do not tune the models):

```{r,eval=TRUE, cache=TRUE,warning=FALSE,echo=FALSE}
set.seed(56)
require(randomForest)
trees <-c("5" = 5, "10" = 10, "20" = 20, "50" = 50, "100" = 100, "200" = 200, "300" = 300, "400" = 400, "500" = 500, "600" = 600)
...
#                                  which.min(model$err.rate[,1]))
    predV<-predict(model,x$validation)
    predT<-predict(model,x$train)
...
```{r,eval=TRUE, cache=TRUE,warning=FALSE,echo=FALSE}
generic.plot(trees, c(0,750), c(.12,.32), "Number of trees", randomForest.c.mce)
```

Poly2 with 200 trees gave the best result with a validation MSE of `r round(randomForest.c.mce$poly2[6], 3)` (`r round(proc.base.class(randomForest.c.mce$poly2[6]), 2)`% lower than baseline).

## 8.6 ADA MODELS

We construct a set of ADA models, our understanding of ADA is lacking but the model partly relies on repeating some training procedure and giving misclassified cases in the i’th iteration higher priority in the i+1’th iteration. Note that we do not tune the models.
...
We construct a set of logistic regression models, these models fit a multivariate sigmoid function to our data points and return the probability of an observation to be true. We vary the cutoff value (how high the probability has to be for an observation to be classified as true) and plot the results:
...
The random forest model with 200 trees for the feature set Poly2 gave the lowest validation MSE.
...
best.class.V <- c("baseline" = baseline.misclass, "KNN" = min(knn.models$poly3), "LDA" = lda.qda.models$poly3$lda.validation.MCE, "QDA" = lda.qda.models$poly3$qda.validation.MCE, "SVM" =  df$validationError[[13]], "RF" = randomForest.c.models$poly2$validationMCE[6], "ADA" = ada.models$poly3$`30`, "Log. reg." = min(1 - performance(x[[2]], "acc")@y.values[[1]]))

barplot(best.class.V, las = 2, col = c(cust.col[1],cust.col[3],cust.col[3],cust.col[3],cust.col[1],cust.col[2],cust.col[3],cust.col[2]), ylim = c(0,0.4), main = "Best Data sets", ylab = "Validation MCE")
...
Predictions<-predict(randomForest.c.models$poly2$model,newdata=testPoly,type="response")
temp<-MCE(Predictions,test$quality>GOOD_WINE)
```

The test MCE was `r round(temp,3)`, `r round(proc.base.class(temp),1)`% under baseline, meaning this model did not satisfy our goal of a 50% reduction in MCE. However, as the learning rate will show, the model will probably perform better if more data is gathered).

```{r,echo=FALSE,message=FALSE}
barplot(c("Training" = randomForest.c.models$poly2$trainMCE[6], "Validation" = min(randomForest.c.models$poly2$validationMCE), "Test" = temp), las = 2, ylab = "MSE", ylim = c(0,0.40))
```

We construct a confusion matrix:

```{r,echo=FALSE} 
table(Predictions,test$quality>GOOD_WINE)
```

$Precision=\frac{562}{562+59}\approx 0.90$

$Recall=\frac{562}{562+104}\approx 0.84$

$Specificity=\frac{199}{199+104}\approx 0.66$

Even though the model did not satisfy our goal, it is pleasant that the model has a high precision (this minimizes the risk of someone expecting to get a good wine but obtaining a poor wine).

## 9.3 LEARNING RATE

We plot the learning rate of the chosen model (note that we do not tune the individual models):
...
l<-lapply(toTest,function(x){
    ...
    model<-randomForest(binary.quality ~ ., data=trainPoly, ntree = 200)
    #tuned.model <- randomForest(binary.quality ~ ., data=trainPoly, ntree =
     #                           which.min(model$err.rate[,1]))
    predValidation<-predict(model,validatePoly)
    predTrain<-predict(model,trainPoly)
...
plot(y=t,x = toTest*nrow(trainValidate),type="l",col="red",ylim=c(0,0.4),ylab="MCE",xlab="Number of observations in training",main="Learning Rate Random Forest")
lines(y=v,x=toTest*nrow(trainValidate),col="blue")
legend("top",legend = c("Train","Validation"),col=c("red","blue"),lwd = 1)
```

The plot is decreasing (although there seems to be a hopefully random increase at the very end), suggesting that more data will improve the model.

# 10. SUMMARY OF THE PROJECT

In both the regression and classification sub tasks, the random forest models performed best: They gave lowest MSE and MCE while not being too computationally expensive. Generally, the normal, poly2 and poly3 feature sets gave the best results.
