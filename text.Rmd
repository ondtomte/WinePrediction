...
knn.plot <- function(x.cord, x.lim, x){
    plot(x.cord, x$normal$mce, col = 1, lwd = 3, type = "l", ylim = c(.17,.33), xlim = x.lim, xlab =
    "Number of neighbors", ylab = "Validation MCE")
    for (i in 2:length(x)){
      lines(x.cord, x[[i]]$mce, col = i, lwd = 3)}
    legend("topright", names(x), col=seq(1,length(x)), lty=1, cex = 0.5)}

# COLOR PROBLEM, rainbow()...?

knn.plot(neighbors, c(0,240), knn.models)
```
...
rf.plot <- function(x.cord, x.lim, x){
    plot(x.cord, x$normal, col = 1, lwd = 3, type = "l", ylim = c(.12,.32), xlim = x.lim,, xlab =
    "Number of trees", ylab = "Validation MCE")
    for (i in 2:length(x)){
      lines(x.cord, x[[i]], col = i, lwd = 3)}
    legend("topright", names(x), col=seq(1,length(x)), lty=1, cex = 0.5)}

rf.plot(trees, c(0,240), randomForest.c.models)
...
ada.plot <- function(x.cord, x.lim, x){
    plot(x.cord, x$normal, col = 1, lwd = 3, type = "l", ylim = c(.18,.33), xlim = x.lim,, xlab =
    "Number of iterations", ylab = "Validation MCE")
    for (i in 2:length(x)){
      lines(x.cord, x[[i]], col = i, lwd = 3)}
    legend("topright", names(x), col=seq(1,length(x)), lty=1, cex = 0.5)}

ada.plot(iterations, c(1,23), ada.models)
