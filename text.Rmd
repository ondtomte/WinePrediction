...
library(MASS)
ridge.models<-lapply(dataSets[-14],function(x){
  #x<- dataSets$poly2
  t<-data.frame(x$train)
  v<-data.frame(x$validation)
  models<-lm.ridge(formula = quality~.,data=t,lambda = seq(0,1500,by=10))
  predictionsValidation <- scale(v,center = models$xm,scale = models$scales)%*%models$coef +models$ym
  validationMSE <- apply(predictionsValidation,MARGIN=2,MSE,y=validation$quality)
  
  predictionsTrain<-  scale(t[-ncol(t)],center = models$xm,scale = models$scales)%*%models$coef +models$ym
